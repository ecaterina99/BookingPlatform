import {writable, derived} from 'svelte/store';
import type {UserDTO, OrganizationMemberDTO} from '$lib/types';

interface AuthState {
    token: string | null;
    user: UserDTO | null;
    memberships: OrganizationMemberDTO[];
}

function createAuthStore() {
    const stored = typeof localStorage !== 'undefined' ? localStorage.getItem('auth') : null;
    const parsed = stored ? JSON.parse(stored) : null;
    const initial: AuthState = parsed ? {...parsed, memberships: parsed.memberships ?? []} : {token: null, user: null, memberships: []};
    const {subscribe, set} = writable<AuthState>(initial);

    return {
        subscribe,
            login(token: string, user: UserDTO, memberships: OrganizationMemberDTO[]) {
                const state = {token, user, memberships};
                localStorage.setItem('auth', JSON.stringify(state));
            set(state);
        },
        logout() {
            localStorage.removeItem('auth');
            set({token: null, user: null, memberships: []});
        }
    };

}

export const auth = createAuthStore();
export const isLoggedIn = derived(auth, $a => $a.token !== null);
export const currentUser = derived(auth, $a => $a.user);
export const isGlobalAdmin = derived(auth, $a => $a.user?.globalRole === 'GLOBAL_ADMIN');
export const isSpecialist = derived(auth, $a => $a.memberships.some(m => m.role === 'SPECIALIST'));